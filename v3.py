...
class App:
    ...
    def pantalla_confirmacion(self):
        self.limpiar_container()
        sel = getattr(self, "ultima_seleccion", None)
        if not sel:
            self.pantalla_menu()
            return

        ttk.Label(self.container, text="✅ Confirmación del pedido", font=("Arial", 16, "bold")).pack(pady=8)
        ...
        if sel["platillos"]:
            texto += "🍽️ Platillos:\n"
            for p, cant in sel["platillos"].items():
                precio_unit = PRECIOS_PLATILLOS.get(p, 0.0)
                subtotal = precio_unit * cant
                texto += f" - {p} x{cant}  = ${subtotal:.2f} (${precio_unit:.2f} c/u)\n"
                total += subtotal
        if sel["bebidas"]:
            texto += "\n🥤 Bebidas:\n"
            for b, cant in sel["bebidas"].items():
                precio_unit = PRECIOS_BEBIDAS.get(b, 0.0)
                subtotal = precio_unit * cant
                texto += f" - {b} x{cant}  = ${subtotal:.2f} (${precio_unit:.2f} c/u)\n"
                total += subtotal

        texto += f"\n💳 Método de pago: {sel['metodo']}\n"
        texto += f"\nTOTAL: ${total:.2f}"

        lbl = tk.Label(frame, text=texto, justify="left", font=("Consolas", 11))
        lbl.pack(pady=6, anchor="w")

        btns = ttk.Frame(self.container, padding=6)
        btns.pack(fill="x")
        ttk.Button(btns, text="🧾 Generar y guardar ticket", command=self.generar_ticket).pack(side="left", padx=6)
        ttk.Button(btns, text="↩️ Volver al menú", command=self.pantalla_menu).pack(side="left", padx=6)

    # -------------------------
    # Generar ticket: guardar en DB + archivo
    # -------------------------
    def generar_ticket(self):
        sel = getattr(self, "ultima_seleccion", None)
        if not sel:
            messagebox.showerror("Error", "No hay selección para generar ticket.")
            return

        # Calcular total y preparar items (con cantidades)
        total = 0.0
        items = []  # (display_name, type, subtotal)
        for p, cant in sel["platillos"].items():
            price_unit = PRECIOS_PLATILLOS.get(p, 0.0)
            subtotal = price_unit * cant
            total += subtotal
            items.append((f"{p} x{cant}", "platillo", subtotal))
        for b, cant in sel["bebidas"].items():
            price_unit = PRECIOS_BEBIDAS.get(b, 0.0)
            subtotal = price_unit * cant
            total += subtotal
            items.append((f"{b} x{cant}", "bebida", subtotal))
        ...
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("INSERT INTO tickets (user_id, total, payment_method, created_at) VALUES (?, ?, ?, ?)",
                    (user_id, total, metodo, created_at))
        ticket_id = cur.lastrowid

        for name, typ, price in items:
            cur.execute("INSERT INTO ticket_items (ticket_id, item_name, item_type, price) VALUES (?, ?, ?, ?)",
                        (ticket_id, name, typ, price))

        conn.commit()
        conn.close()

        # Guardar en archivo de texto
        texto_ticket = f"--- Ticket #{ticket_id} ---\nUsuario: {self.current_user[1]}\nFecha(UTC): {created_at}\n\n"
        texto_ticket += "Items:\n"
        for name, typ, price in items:
            texto_ticket += f" - {name} ({typ}) : ${price:.2f}\n"
        texto_ticket += f"\nMétodo de pago: {metodo}\nTOTAL: ${total:.2f}\n----------------------"
        append_ticket_to_file(texto_ticket)

        messagebox.showinfo("Ticket guardado", f"Ticket generado y guardado (ID: {ticket_id}).")
        # Mostrar ticket final
        self.pantalla_ticket_final(ticket_id, texto_ticket)

    # -------------------------
    # Pantalla: Ticket final (visual)
    # -------------------------
    def pantalla_ticket_final(self, ticket_id: int, texto_ticket: str):
        self.limpiar_container()

        ttk.Label(self.container, text=f"🧾 Ticket #{ticket_id}", font=("Arial", 16, "bold")).pack(pady=8)

        text_widget = tk.Text(self.container, height=15, wrap="word", font=("Consolas", 11))
        ...
        btns = ttk.Frame(self.container, padding=6)
        btns.pack(fill="x")
        ttk.Button(btns, text="🏁 Finalizar (volver al inicio)", command=self.pantalla_inicio_postlogin).pack(side="left", padx=6)
        ttk.Button(btns, text="📂 Abrir carpeta de tickets", command=self.abrir_carpeta_tickets).pack(side="left", padx=6)
        ttk.Button(btns, text="↩️ Hacer otro pedido", command=self.pantalla_menu).pack(side="right", padx=6)
    ...
    def abrir_carpeta_tickets(self):
        ...
        try:
            ...
            if os.name == "nt":
                ...
                # plataforma unix: intentar con xdg-open o solo mostrar ruta
                try:
                    os.system(f'xdg-open "{path}"')
                except Exception:
                    messagebox.showinfo("Abrir carpeta", f"Ve a: {path}")
    ...
    def mostrar_tickets_guardados(self):
        # Abrir ventana con lista de tickets desde DB
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT t.id, u.username, t.total, t.payment_method, t.created_at FROM tickets t LEFT JOIN users u ON t.user_id = u.id ORDER BY t.id DESC LIMIT 100")
        rows = cur.fetchall()
        conn.close()

        win = tk.Toplevel(self.root)
        win.title("Tickets guardados (últimos 100)")
        win.geometry("820x420")

        tree = ttk.Treeview(win, columns=("id", "user", "total", "method", "created"), show="headings")
        tree.heading("id", text="ID")
        tree.heading("user", text="Usuario")
        tree.heading("total", text="Total")
        tree.heading("method", text="Pago")
        tree.heading("created", text="Fecha (UTC)")
        tree.pack(fill="both", expand=True)

        for r in rows:
            tree.insert("", "end", values=(r[0], r[1] if r[1] else "Anon", f"${r[2]:.2f}", r[3], r[4]))
        ...
        def ver_detalle(event=None):
            ...
            conn = get_db_connection()
            cur = conn.cursor()
            cur.execute("SELECT item_name, item_type, price FROM ticket_items WHERE ticket_id = ?", (ticket_id,))
            items = cur.fetchall()
            cur.execute("SELECT total, payment_method, created_at FROM tickets WHERE id = ?", (ticket_id,))
            tmeta = cur.fetchone()
            conn.close()

            texto = f"Ticket #{ticket_id}\nFecha: {tmeta[2]}\nPago: {tmeta[1]}\nTotal: ${tmeta[0]:.2f}\n\nItems:\n"
            for it in items:
                texto += f" - {it[0]} ({it[1]}) : ${it[2]:.2f}\n"

            detail_win = tk.Toplevel(win)
            detail_win.title(f"Detalle Ticket #{ticket_id}")
            txt = tk.Text(detail_win, width=60, height=20)
    ...
    # -------------------------
    # Pantalla inicio (después de login)
    # -------------------------
    def pantalla_inicio_postlogin(self):
        self.limpiar_container()
        ttk.Label(self.container, text="🍽️ ¡Bienvenido al restaurante!", font=("Arial", 18, "bold")).pack(pady=12)
        ttk.Label(self.container, text=f"Usuario actual: {self.current_user[1] if self.current_user else 'Invitado'}", font=("Arial", 11)).pack(pady=6)

        ttk.Button(self.container, text="🛒 Hacer pedido", command=self.pantalla_menu).pack(pady=8)
        ttk.Button(self.container, text="🧾 Ver tickets guardados", command=self.mostrar_tickets_guardados).pack(pady=8)
        ttk.Button(self.container, text="🔒 Cerrar sesión", command=self.logout).pack(pady=8)
